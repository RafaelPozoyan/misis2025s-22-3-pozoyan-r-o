    ...
    std::vector<cv::Point2f>& quad)
{
    ...
    Point2f tl, tr, bl;
    // Находим точку с прямым углом (две короткие стороны)
    if (d01 > d02 && d01 > d12) { // 0-1 самая длинная
        tl = triple[2];
        tr = (triple[0].y < triple[1].y) ? triple[0] : triple[1];
        bl = (triple[0].y > triple[1].y) ? triple[0] : triple[1];
    }
    else if (d02 > d01 && d02 > d12) { // 0-2 самая длинная  
        tl = triple[1];
        tr = (triple[0].y < triple[2].y) ? triple[0] : triple[2];
        bl = (triple[0].y > triple[2].y) ? triple[0] : triple[2];
    }
    else { // 1-2 самая длинная
        tl = triple[0];
        tr = (triple[1].y < triple[2].y) ? triple[1] : triple[2];
        bl = (triple[1].y > triple[2].y) ? triple[1] : triple[2];
    }

    // Убеждаемся в правильном порядке left/right
    if (tr.x < bl.x) std::swap(tr, bl);

    // Определяем размер модуля через timing patterns
    float moduleSize;
    if (!ImageProcessor::findTimingPatterns(bin, tl, tr, bl, moduleSize)) {
        // Fallback - оцениваем по расстоянию между finder patterns
        float avgDist = (cv::norm(tr - tl) + cv::norm(bl - tl)) / 2.0f;
        moduleSize = avgDist / 21.0f;
    }

    // Вычисляем точные координаты углов с учетом quiet zone (4 модуля)
    Point2f rightVec = (tr - tl) / 21.0f;  // вектор на один модуль вправо
    Point2f downVec = (bl - tl) / 21.0f;   // вектор на один модуль вниз

    // Добавляем quiet zone (по 4 модуля с каждой стороны)
    Point2f quietZone = rightVec * 4.0f + downVec * 4.0f;

    Point2f realTL = tl - rightVec * 4.0f - downVec * 4.0f;        // top-left с quiet zone
    Point2f realTR = tr + rightVec * 4.0f - downVec * 4.0f;        // top-right с quiet zone  
    Point2f realBL = bl - rightVec * 4.0f + downVec * 4.0f;        // bottom-left с quiet zone
    Point2f realBR = tr + rightVec * 4.0f + bl - tl + downVec * 4.0f; // bottom-right с quiet zone

    // Альтернативный расчет BR через векторную геометрию
